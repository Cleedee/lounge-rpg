    ...
    >
      <div className="border border-[#00ffcc]/40 rounded-lg bg-[#0a0a0f] p-6 w-[680px] max-w-full mx-4 glow-teal">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[#00ffcc] font-bold text-lg tracking-widest text-glow-teal">
              {t("class_selector.title", "SELECT YOUR CLASS")}
            </h2>
            <p className="text-[#6644aa] text-xs mt-0.5">{t("class_selector.subtitle", "Choose your operative for Neo-Pawsburg")}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <p className="text-[#6644aa]/50 text-xs text-center mt-4">
          {t("class_selector.tip", "Tip: SphinxRogue (Sandpaw) is ideal for demo — high AGI and STL make dice rolls exciting.")}
        </p>
      </div>
    </div>
  );
