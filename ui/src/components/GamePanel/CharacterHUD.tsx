    ...
    >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[#6644aa] text-xs uppercase tracking-wider">
                {meta ? t(`class.${meta.labelKey}`, meta.label) : t("hud.select_class", "Select a class")}
              </div>
            </div>
          </div>
          <button
      ...
      {stats && (
        <>
          <div className="space-y-1.5 mb-2">
            <StatBar current={stats.hp} max={stats.maxHp} label={t("hud.hp", "HP")} />
            <div className="flex items-center gap-2 text-xs">
              <span className="text-[#6644aa] w-6 shrink-0">{t("hud.xp", "XP")}</span>
              <div className="relative flex-1 h-2 bg-[#0d0d18] rounded border border-[#1a1a2e] overflow-hidden">
                <div
          ...
          <div className="flex items-center justify-between text-xs mb-2">
            <span className="text-[#ffaa00]">
              💰 {stats.gold} {t("hud.gold", "CreditChips")}
            </span>
      ...
      {!stats && state.isProcessing && (
        <div className="text-center py-2">
          <span className="text-[#6644aa] text-xs">{t("app.loading", "Loading")}</span>
      ...
      {!stats && !state.isProcessing && (
        <div className="text-center py-2">
          <span className="text-[#6644aa] text-xs">{t("hud.no_campaign", "No active campaign — ")}</span>
          <button
            onClick={onNewGame}
            className="text-[#00ffcc] text-xs underline underline-offset-2 hover:text-glow-teal"
          >
            {t("hud.pick_class", "pick a class to start")}
          </button>
        </div>
      )}
    </div>
