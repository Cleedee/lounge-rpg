    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[#00ffcc] font-bold text-sm group-hover:text-glow-teal">
          {t(`class.${meta.labelKey}`, meta.label).toUpperCase()}
        </span>
      </div>

      <p className="text-[#c8ccd4]/70 text-xs mb-3 leading-relaxed">{t(`class.${meta.descriptionKey}`, meta.description)}</p>
      ...
      <div className="border-t border-[#1a1a2e] pt-2">
        <span className="text-[#ff00aa]/70 text-xs">✦ {t(`class.${meta.abilityKey}`, meta.ability)}</span>
      </div>
    </button>
  );
}
